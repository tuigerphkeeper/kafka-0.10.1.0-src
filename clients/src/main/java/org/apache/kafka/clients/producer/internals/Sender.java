...
public class Sender implements Runnable {
    ...
    void run(long now) {
        // 获取元数据
        // 目前是第一次代码进来，还没有获取到元数据
        // cluster为null
        // 接下来的代码以来元数据，所以暂时不用看下面的代码

        /**
         * 第二次代码进来，已经有元数据了，所以cluster这儿是有元数据
         *
         * 步骤一：
         *      获取元数据
         */
        ...
        /**
         * 步骤二：
         *      首先判断哪些partition有消息可以发送，获取到这个partition的leader partition对应的broker主机
         */
        ...
        while (iter.hasNext()) {
            ...
            if (!this.client.ready(node, now)) {
                // 如果返回的是false，移除result里面要发送消息的主机
                // 所以这里所有的主机都会被移除
                iter.remove();
        ...
        /**
         * 步骤五：
         *      我们有可能要发送的partition有很多个，有一些partition的leader partition在同一台服务器上
         *
         * 当我们分区的个数大于集群节点的个数时，一定有多个leader partition在同一台服务器上
         *
        ...
        /**
         * 步骤八：
         *      真正执行网络操作的都是这个NetWorkClint组件，包括发送请求，接受请求
         */
        // 这里可能就是建立网络连接的代码
        ...
        if (response.wasDisconnected()) {
            ...
            if (response.hasResponse()) {
                ...
                for (Map.Entry<TopicPartition, ProduceResponse.PartitionResponse> entry : produceResponse.responses().entrySet()) {
                    ...
                    // 如果处理成功那就是成功了
                    // 如果服务端处理失败了，也要给我们发送回来异常的信息
                    // error里面存储的就是服务端发送回来的异常码
    ...
    private void completeBatch(RecordBatch batch, Errors error, long baseOffset, long timestamp, long correlationId, long now) {
        ...
        if (error != Errors.NONE && canRetry(batch, error)) {
            ...
            // 回调函数调用了以后，说明我们的一个完整的消息发送流程就结束了
    ...
    private ClientRequest produceRequest(long now, int destination, short acks, int timeout, List<RecordBatch> batches) {
        ...
        RequestCompletionHandler callback = new RequestCompletionHandler() {
            public void onComplete(ClientResponse response) {
                // 回调函数要是被调用，其实就是这个方法被执行了
                handleProduceResponse(response, recordsByPartition, time.milliseconds());
            }
        };
        // 这个就是我们封装出来的请求，里面带了一个回调函数
