...
public class NetworkClient implements KafkaClient {
    ...
    public boolean isReady(Node node, long now) {
        ...
        // priority
        /**
         * isUpdateDue：发送写数据请求的时候，不能是正在更新元数据的时候
         * canSendRequest：是否能发送
         */
    ...
    private boolean canSendRequest(String node) {
        /**
         * isConnected：
         *      生产者缓存多个连接（和broker的节点数是一样的）
         *      判断缓存里面是否已经把这个连接建立好了
         * isChannelReady：
         *      java NIO：Selector
         *      Selector -> 绑定多个kafkaChannel（java SocketChannel）
         *      一个KafkaChannel就代表了一个连接
         *      判断kafka底层网络连接是否建立好了
         * canSendMore：
    ...
    private void doSend(ClientRequest request, long now) {
        ...
        // 这里往inFlightRequests里面存request请求，存储的就是还没有收到响应的请求
        // 默认最多能存5个请求
        // 如果请求发送出去了，也成功接受到了响应，后面就会把存在这里的请求删除
    ...
    public List<ClientResponse> poll(long timeout, long now) {
         ...
         * 步骤一：
         *      封装了一个要拉取元数据的请求
         * 在这个方法里面设计到Kafka网络的方法，等分析完kafka的网络设计后，再回头看这儿的代码
         */
         ...
         * 获取元数据 -> 判断网络是否建立好 -> 建立网络连接 -> 发送请求（获取元数据请求）-> 服务端发送回来响应（带了集群的饿元数据信息）
         */
        ...
        for (ClientResponse response : responses) {
            if (response.request().hasCallback()) {
                try {
                    // 调用响应里面封装的请求的回调函数
                    // TODO 猜测当时封装网络请求的时候，肯定是给它绑定了一个回调函数
    ...
    private void processDisconnection(List<ClientResponse> responses, String nodeId, long now) {
        ...
        for (ClientRequest request : this.inFlightRequests.clearAll(nodeId)) {
            ...
            if (!metadataUpdater.maybeHandleDisconnection(request))
                // 对这些请求进行处理
                // 自己封装了一个响应，这个响应里面没有服务端响应消息（服务端没有给响应）
                // 失去连接的状态表示为true
    ...
    private void handleCompletedReceives(List<ClientResponse> responses, long now) {
        for (NetworkReceive receive : this.selector.completedReceives()) {
            ...
            // 在inFlightRequests中移除已经接收到了响应的请求
            // 把之前存入进去的请求也获取到了
            ClientRequest req = inFlightRequests.completeNext(source);
            // 解析服务端发送回来的响应（里面有响应的结果数据）
            Struct body = parseResponse(receive.payload(), req.request().header());
            // TODO 如果是关于元数据信息的响应
            if (!metadataUpdater.maybeHandleCompletedReceive(req, now, body))
                // 解析完以后封装成一个一个的ClientResponse
                // req：发送出去的请求信息
                // body：解析后的响应的内容
    ...
    class DefaultMetadataUpdater implements MetadataUpdater {
        ...
        private void maybeUpdate(long now, Node node) {
            ...
            // 判断网络连接是否建立好
            // 因为还没有学习Kafka网络设计，所以暂时认为这里网络已经建立好了
            if (canSendRequest(nodeConnectionId)) {
                this.metadataFetchInProgress = true;
                MetadataRequest metadataRequest;
                if (metadata.needMetadataForAllTopics())
                    // 封装请求（获取所有topic）的元数据信息的请求
